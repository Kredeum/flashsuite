...
aaveDashboard2.getUserData = async function (_user, _provider, _log = false) {
  ...
  const answerGQL = await _graphQL(url, query);
...
aaveDashboard2.getRiskParameters = async function (_positions, _mode = 0, _log = false) {
...
aaveDashboard2.getHealthFactors = async function (_positions, _log = false) {
  const healthFactor = (await aaveDashboard2.getRiskParameters(_positions, 0)).healthFactor;
  const healthFactorChecked = (await aaveDashboard2.getRiskParameters(_positions, 1)).healthFactor;
  const healthFactorUnchecked = (await aaveDashboard2.getRiskParameters(_positions, 2)).healthFactor;
  return { healthFactor, healthFactorUnchecked, healthFactorChecked };
}
aaveDashboard2.getHealthFactors2 = async function (_positionsOrigin, _positionsDestination, _log = false) {
  ...
  return aaveDashboard2.getHealthFactors(positions, _log);
}
