...
const priceEvolPer100000 = 1; // 0,001 %
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
FlashAccounts.approveTransfer = async function (_position, _signer) {
  const aTokenContract = new ethers.Contract(_position.address, ERC20.ABI, _signer);
  return aTokenContract.approve(FlashAccounts.contract.address, _plusEvol(_position.amount));
}
  ...
  for await (const position of _dashboard) {
    if (position.type == 0) {
      ...
      if (FlashAccounts.log) {
        console.log(`TX1.${++ia} allow transfer ${_bal(_position.amount, _position.decimals)} ${_position.symbol}\n${ethscan}/tx/${txMain.hash}`);
      }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
FlashAccounts.approveLoan = async function (_position, _signer) {
  ...
  return debtTokenContract.approveDelegation(FlashAccounts.contract.address, _plusFee(_plusEvol(_position.amount)));
}
FlashAccounts.approveLoans = async function (_dashboard, _signer) {
  ...
  for await (const position of _dashboard) {
    if (_position.type > 0) {
      const txMain = await FlashAccounts.approveLoan(position, _signer);

      if (FlashAccounts.log) {
        console.log(`TX2.${++id} Allow borrow ${_bal(_position.amount, _position.decimals)} ${_position.symbol}\n${ethscan}/tx/${txMain.hash}`);
      }
      const txResult = await txMain.wait();
      if (FlashAccounts.log) {
        console.log(txResult);
...
FlashAccounts.ADDRESS = {
  'mainnet': '',
  'kovan': '0xdeAadaf7aDA7eF21f08d7Fb1D0081c0281dbBE48'
};
FlashAccounts.ABI = [
  {
    "inputs": [
