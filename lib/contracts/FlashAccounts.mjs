...
function _plus(_n, _percentmille = 1) {
  const ret = BigNumber.from(_n).mul(100000 + _percentmille).div(100000).toString();
  console.log("_plus", _n, _percentmille, ret);
  return ret;
}
...
FlashAccounts.approveTransfer = async function (_position, _signer, _index = 1) {
  ...
  return aTokenContract.approve(FlashAccounts.contract.address, _plus(_position.amount, 100));
}
...
FlashAccounts.approveLoan = async function (_position, _signer, _index = 1) {
  ...
  return debtTokenContract.approveDelegation(FlashAccounts.contract.address, _plus(_position.amount, 100));
}
...
FlashAccounts.callFlashLoanTx = async function (_dashboard, _from, _to, _signer) {
  ...
  for await (const position of _dashboard) {
    if (position.type == 0) {
      ...
      dTokenAmounts.push(_plus(position.amount, 1));
...
FlashAccounts.ADDRESS = {
  ...
  // 'kovan': '0x8BF9544D45B0F52b7e036DD2c7293C85915a0793'
  'kovan': '0x67Bf74a88c5785eBE65dc29757dfE99058d9Ae8D'
};
FlashAccounts.ABI = [
  {
    "inputs": [
