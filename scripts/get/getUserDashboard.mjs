...
async function aaveDashboard(_address) {
  const assets = [], amounts = [], tokens = [], types = [];
  ...
  for (const reserve of reserveTokens) {
    ...
    if (aBal > 0 || sdBal > 0 || vdBal > 0) {
      ...
      if (aBal > 0) {
        assets.push(aToken); amounts.push(aBal.toString()); tokens.push('a' + reserve.symbol); types.push('a');
        console.log(aToken, aBal.toString(), 'a' + reserve.symbol,)
      }
      if (sdBal > 0) {
        assets.push(sdToken); amounts.push(sdBal.toString()); tokens.push('sd' + reserve.symbol); types.push('sd');
        console.log(sdToken, sdBal.toString(), 'sd' + reserve.symbol,)
      }
      if (vdBal > 0) {
        assets.push(vdToken); amounts.push(vdBal.toString()); tokens.push('vd' + reserve.symbol); types.push('vd');
        console.log(vdToken, vdBal.toString(), 'vd' + reserve.symbol,)
      }
    }
  };
  return { assets, amounts, tokens, types };
}
