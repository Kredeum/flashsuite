    ...
    try {
      if (nd > 0) {
        ...
        for await (const tx of txsWait) {
          ...
          message = `Waiting transactions completion... ${
            ia + 1
          }/${nd} deposit${ia > 1 ? "s" : ""} completed`;
      ...
      <div class="columnspositions fs-columnspositions w-row">
        ...
        {#key refresh}
          <Dashboard
            ...
            bind:reget
            bind:healthFactorChecked={healthFactorUncheckedBob}
          />
          <Dashboard
            address={Bob}
            name="Destination"
            bind:reget
            bind:healthFactorUnchecked={healthFactorUncheckedBob}
          />
        {/key}
      </div>
      <div class="w-100">
